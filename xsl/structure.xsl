<xsl:stylesheet 
  ...
  <xsl:template match="@id">
    ...
    <xsl:if test="not($strip-id)">
      <!-- IDs from the source document are always prefixed with "id-" to avoid clashes
           with IDs generated by the XSLT rules.  Of course this escaping does not apply
           to the "re-process" template mode described above.  
           
           That said, we do not expect many IDs coming from the source document. IDs are
           typically used for navigation (e.g. table of contents) or for targeting 
           elements with CSS or JavaScript.  Generally such features are tedious to
           key in by hand and have to be generated through the XSLT rules here, anyway. 
           -->
      <xsl:attribute name="id" select="concat('id-', string(.))" />
    </xsl:if>
  </xsl:template>       
